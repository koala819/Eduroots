...
interface BehaviorEditProps {
  ...
  initialData: {
    ...
    behaviorRecords: { [key: string]: number }
    behaviorId: string
  }
}

export function BehaviorEdit({
  courseSessionId,
  date,
  behaviorId,
  students,
  ...
  const { updateBehaviorRecord } = useBehavior()
  const [behaviorRecords, setBehaviorRecords] = useState(initialData.behaviorRecords)
  ...
  async function handleSave() {
    ...
    try {
      ...
      // Mise à jour d'un behavior existant
      ...
      console.error('❌ [BehaviorEdit] Erreur sauvegarde:', error)
      setError('Erreur lors de la mise à jour des comportements')
    } finally {
      setIsUpdating(false)
    }
  }
  ...
  return (
    <div className="w-full max-w-4xl mx-auto">
      ...
      <section className="bg-background border border-border rounded-lg shadow-sm p-4 sm:p-6 mb-6">
        <div id="behavior-edit-description" className="sr-only">
          Formulaire de modification des comportements pour la session du{' '}
