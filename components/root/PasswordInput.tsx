...
interface PasswordInputProps {
  ...
  setShowPwd: React.Dispatch<React.SetStateAction<Record<'student' | 'teacher', boolean>>>
}
...
export const PasswordInput: React.FC<PasswordInputProps> = ({
  ...
  <div className="space-y-2">
    <div className="flex items-center space-x-2">
      ...
      <span>{value === '[DÉFINI]' ? 'Mot de passe défini' : 'Mot de passe non défini'}</span>
    </div>
    <div className="relative">
      <Input
        value={value}
        type={showPwd[type] ? 'text' : 'password'}
        placeholder={value === '[DÉFINI]' ? 'Mot de passe défini' : 'Nouveau mot de passe'}
