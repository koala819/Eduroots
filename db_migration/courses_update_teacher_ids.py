...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('update_teacher_ids.log'),
...
def add_teacher_id_column(pg_conn):
    """Ajoute la colonne teacher_id à la table tmp_courses_teacher"""
    try:
        cur = pg_conn.cursor()
        ...
        cur.execute("""
            ...
            AND table_name = 'tmp_courses_teacher'
        ...
        if not cur.fetchone():
            # Ajouter la colonne
            cur.execute("""
                ALTER TABLE education.tmp_courses_teacher
                ADD COLUMN teacher_id UUID REFERENCES education.users(id)
            """)
            pg_conn.commit()
            logger.info("Colonne teacher_id ajoutée avec succès")
        else:
            logger.info("La colonne teacher_id existe déjà")
    ...
    try:
        ...
        for record in records:
            ...
            if mongo_teacher_id in id_mapping:
                ...
                cur.execute("""
                    UPDATE education.tmp_courses_teacher
                    SET teacher_id = %s
                    WHERE id = %s
                ...
                logger.warning(f"ID MongoDB non trouvé dans le mapping: {mongo_teacher_id}")
                not_found_count += 1
...
def main():
    try:
        ...
        # Ajouter la colonne teacher_id
        add_teacher_id_column(pg_conn)
