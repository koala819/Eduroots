...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('update_teacher_stats_id.log'),
...
def update_teacher_stats_id(pg_conn):
    """Met à jour le champ teacher_stats_id dans education.users"""
    try:
        cur = pg_conn.cursor()

        # Compter le nombre d'enseignants à mettre à jour
        cur.execute("""
            SELECT COUNT(*)
            FROM education.users u
            JOIN stats.teacher_stats ts ON u.id = ts.user_id
            WHERE u.role = 'teacher'
            AND (u.teacher_stats_id IS NULL OR u.teacher_stats_id != ts.id)
        """)
        count = cur.fetchone()[0]
        logger.info(f"Nombre d'enseignants à mettre à jour: {count}")

        # Mettre à jour les teacher_stats_id
        cur.execute("""
            UPDATE education.users u
            SET teacher_stats_id = ts.id
            FROM stats.teacher_stats ts
            WHERE u.id = ts.user_id
            AND u.role = 'teacher'
            AND (u.teacher_stats_id IS NULL OR u.teacher_stats_id != ts.id)
            RETURNING u.id, u.teacher_stats_id
        """)

        updated_users = cur.fetchall()

        if updated_users:
            logger.info(f"Nombre d'enseignants mis à jour: {len(updated_users)}")
            for user_id, stats_id in updated_users:
                logger.info(f"Enseignant {user_id} mis à jour avec stats_id {stats_id}")
        else:
            logger.info("Aucun enseignant à mettre à jour")
    ...
    try:
        ...
        if remaining_errors:
            ...
            for user_id, stats_id, correct_stats_id in remaining_errors:
                logger.error(f"Incohérence trouvée: user_id={user_id}, teacher_stats_id={stats_id}, correct_stats_id={correct_stats_id}")
            return False
        else:
...
def main():
    ...
    try:
        ...
        # Mettre à jour les teacher_stats_id
        if update_teacher_stats_id(pg_conn):
            logger.info("Mise à jour des teacher_stats_id terminée")
